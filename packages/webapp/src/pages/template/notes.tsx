...
const Notes = () => {
  ...
  const saveNote = () => {
    ...
    if (params.id) {
      // update task
      const note: any = notesList.find((d: any) => d.id === params.id);
  ...
  const setFav = (note: any) => {
    ...
    const item = list.find((d: any) => d.id === note.id);
  ...
  const setTag = (note: any, name: string = '') => {
    const list = filterdNotesList;
    const item = filterdNotesList.find((d: any) => d.id === note.id);
    item.tag = name;
    setFilterdNotesList([...list]);
    if (selectedTab !== 'all' || selectedTab === 'delete') {
      searchNotes();
    ...
    <div>
      <div className="relative flex h-full gap-5 sm:h-[calc(100vh_-_150px)]">
        <div
          ...
          onClick={() => setIsShowNoteMenu(!isShowNoteMenu)}></div>
        <div
          className={`panel
                    absolute
                    z-10
                              ...
                              <ul className="text-sm font-medium">
                                <li>
                                  <button type="button" onClick={() => editNote(note)}>
                                    <svg
                                      ...
                                      fill="none"
                                      xmlns="http://www.w3.org/2000/svg"
                                      className="h-4 w-4 shrink-0 ltr:mr-3 rtl:ml-3">
                                      <path
                                ...
                                <li>
                                  <button type="button" onClick={() => deleteNoteConfirm(note)}>
                                    <svg
                                      className="shrink-0 ltr:mr-3 rtl:ml-3"
                                      width="18"
                                      height="18"
                                      viewBox="0 0 24 24"
                                      fill="none"
