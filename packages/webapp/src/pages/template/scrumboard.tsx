...
const Scrumboard = () => {
  ...
  const saveProject = () => {
    ...
    if (params.id) {
      ...
      // add project
      const lastId =
        projectList.reduce((max: number, obj: any) => (obj.id > max ? obj.id : max), projectList[0].id) || 0;
  ...
  const saveTask = () => {
    ...
    if (paramsTask.id) {
      ...
      // add task
      let maxId = 0;
      maxId = project.tasks?.length
        ? project.tasks.reduce((max: number, obj: any) => (obj.id > max ? obj.id : max), project.tasks[0].id)
        : 0;
  ...
  const deleteTask = () => {
    const project = projectList.find((d: any) => d.id === selectedTask.projectId);
